...
flowchart LR
    ...
    B -->|mulai dikerjakan di branch NIU| C[In progress]
...
| In progress | Pemilik issue mulai commit di branch NIU miliknya | Pemilik issue |
...
- Satu anggota hanya boleh punya satu issue berstatus In progress dan satu issue berstatus In review pada saat yang sama.
...
Setiap anggota bekerja di branch NIU miliknya yang sudah dibuat sejak Lab 2.1.

| Anggota | Branch |
|---|---|
| Nabil Aufa Danaputra | `535223` |
| Nayla Thalita | `535820` |
| Gilbert S. H. Nainggolan | `543841` |

Aturan:

- Hanya pemilik yang boleh commit dan push ke branch NIU-nya.
- Tidak ada commit langsung ke `main`. Semua perubahan masuk lewat pull request dari branch NIU ke `main`.
- Satu pull request untuk satu issue. Jika sebuah issue terlalu besar, pecah issue-nya dulu.
- Branch NIU tidak pernah dihapus.

### Menyinkronkan dengan `main`

Branch NIU dipakai sepanjang semester, sehingga mudah tertinggal dari `main` dan memicu konflik. Sinkronkan sebelum mulai mengerjakan issue baru, dan setiap kali pull request anggota lain di-merge:

```
git checkout <nomor-niu>
git pull origin main
git push origin <nomor-niu>
```

Jika muncul konflik, selesaikan di branch NIU sendiri, bukan di `main`.

### Satu pull request terbuka per anggota

Semua commit baru di branch NIU otomatis ikut masuk ke pull request yang sedang terbuka dari branch itu. Karena itu:
